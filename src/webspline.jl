...
function web_splines!(spline_storage::AbstractBasisSplineStorage2D, mpmgrid::MPMGrid{2}, particles::Particles)
    ...
    for j ∈ findall(unstable)
        ...
        stable_splines = grid_splines[closest_stable_grid_cell]
        ...
        e_ij = (kron(ones(ndof(mpmgrid.splines[2])), e_ij_1) .* kron(e_ij_2, ones(ndof(mpmgrid.splines[1]))))[stable_splines]
        # display(e_ij)
        spline_storage.B[:, stable_splines] += spline_storage.B[:, j] * e_ij'
        spline_storage.dB1[:, stable_splines] += spline_storage.dB1[:, j] * e_ij'
        spline_storage.dB2[:, stable_splines] += spline_storage.dB2[:, j] * e_ij'
    end
...
function compute_ak(spline::BasisSpline, rtaylor::Real)
    ...
    a_m = reduce(hcat, deriv) ./ factorial.(0:mpmgrid.splines[1].degree)'
...
function compute_webspline_values!(storage::AbstractBasisSplineStorage2D, mpmgrid::MPMGrid{2}, particles::Particles{2, np}) where np
