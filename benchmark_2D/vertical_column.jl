...
function graviation(t::Real, model::Model{2}, particles::Particles{2, np}, mpmgrid::MPMGrid{2}, splines::BasisSplineStorage) where np
    global g
    return [zeros(ndof(mpmgrid)) splines.B'*(particles.mass .* -g)]
end

mpmgrid = MPMGrid((0, L), nix, degree, (0, H), niy, degree)
particles = initialize_uniform_particles(mpmgrid, ρ, (nix -1) * nppc, (niy - 1) * nppc)
dirichlet = get_boundary_indices(mpmgrid; fix_bottom = true)#, fix_top=true, fix_bottom=true)
...
u_true = u_analytic.(yinit, tend, ρ, g, E, H)
σ_true = σ_analytic.(yinit, tend, ρ, g, E, H)
figu = plot(yinit, particles.displacement[:, 2])
plot!(figu, yinit, u_true)

figs = plot(yinit, particles.σ[:, 4])
plot!(figs, yinit, σ_true)
