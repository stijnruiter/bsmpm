...
oppX = oppBspline(0, L, nix, degree)
...
function corner_traction_force(t::Real, model::Model{2}, particles::Particles{2, np}, mpmgrid::MPMGrid{2}, splines::AbstractBasisSplineStorage) where np
    ...
    compute_bspline_values!(traction_storage, bottom, mpmgrid)
    if model.runparams.spline == :webspline
        web_splines!(traction_storage, mpmgrid, particles.position[particles.bel, :])
    end
    ...
    compute_bspline_values!(traction_storage, left, mpmgrid)
    if model.runparams.spline == :webspline
        web_splines!(traction_storage, mpmgrid, particles.position[particles.bel, :])
    end
    ...
    compute_bspline_values!(traction_storage, top+analytic_displacement(model, top, t, ue, L, H), mpmgrid)
    if model.runparams.spline == :webspline
        web_splines!(traction_storage, mpmgrid, particles.position[particles.bel, :])
    end
    ...
    compute_bspline_values!(traction_storage, right+analytic_displacement(model, right, t, ue, L, H), mpmgrid)
    if model.runparams.spline == :webspline
        web_splines!(traction_storage, mpmgrid, particles.position[particles.bel, :])
    end
...
function penalty_dirichlet_force(t::Real, model::Model{2}, particles::Particles{2, np}, mpmgrid::MPMGrid{2}, splines::AbstractBasisSplineStorage) where np
    ...
    xb = s * L
    yb = zeros(nb)
    compute_bspline_values!(traction_storage, [xb yb], mpmgrid)
    if model.runparams.spline == :webspline
        web_splines!(traction_storage, mpmgrid, particles.position[particles.bel, :])
    end
...
function error_analysis(t::Real, model::Model{2}, particles::Particles{2, np}, mpmgrid::MPMGrid{2}, splines::AbstractBasisSplineStorage) where np
...
mpmgrid = MPMGrid((0, L+5*ue), nix, degree, (0, H+5*ue), niy, degree)
...
traction_storage = initialize_spline_storage(1000, mpmgrid)
